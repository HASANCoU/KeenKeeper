...
const Friends = ({friend}) => {
    ...
    return (
        <div className='bg-white shadow-2xs text-center space-y-2 p-6 rounded-2xl'>
            <div><Image className='rounded-full mx-auto' src={picture} alt={name} height={50} width={50}></Image></div>
            <h2 className='text-[#1F2937] font-semibold'>{name}</h2>
            <p className='text-xs text-[#64748B]'>{days_since_contact}d ago</p>
            <div className='space-x-2'>
                {tags.map((tag,idx)=><div key={idx} className="badge bg-[#CBFADB] text-[#244D3F] border-none">{tag}</div>)}
            </div>
            <div
